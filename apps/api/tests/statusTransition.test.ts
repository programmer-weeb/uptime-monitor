...
describe('recordCheckTransition', () => {
  it('fires exactly one down alert after the second consecutive failure', async () => {
    ...
    const monitorWithUser = { ...monitor, user: { email: user.email } };
    ...
    const second = await recordCheckTransition({
      monitor: { ...secondMonitor, user: { email: user.email } },
      result: {
        status: 'down',
        statusCode: 503,
        latencyMs: 110,
    ...
    const third = await recordCheckTransition({
      monitor: { ...thirdMonitor, user: { email: user.email } },
      result: {
        status: 'down',
        statusCode: 503,
        latencyMs: 120,
  ...
  it('fires one recovery alert when a down monitor returns up', async () => {
    ...
    const transition = await recordCheckTransition({
      monitor: { ...downMonitor, user: { email: user.email } },
      result: {
        status: 'up',
        statusCode: 200,
        latencyMs: 42,
    ...
    await expect(
      prisma.$transaction(async (tx) => {
        await writeCheckTransition(tx, {
          monitor: { ...failingMonitor, user: { email: user.email } },
          result: {
            status: 'down',
            statusCode: 503,
            latencyMs: 100,
